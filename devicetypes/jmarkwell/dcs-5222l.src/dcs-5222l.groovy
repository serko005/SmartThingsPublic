...
metadata {
    ...
    preferences {
        ...
        input("CameraPresetOne", "number", title:"Camera Preset 1", description: "Please enter which preset view number you want to use (other than 'Home')", defaultValue: 1, required: true, displayDuringSetup: true)
        input("CameraPresetTwo", "number", title:"Camera Preset 2", description: "Please enter which preset view number you want to use (other than 'Home')", defaultValue: 2, required: false, displayDuringSetup: true)
        input("CameraPresetThree", "number", title:"Camera Preset 3", description: "Please enter which preset view number you want to use (other than 'Home')", defaultValue: 3, required: false, displayDuringSetup: true)
    }
...
def up() {
    state.requestedPosition = "unknown"
    
    log.debug "Camera moving up."
...
def down() {
    ...
    log.debug "Camera moving down."
    
    moveCmd(0,-5)
}

def home() {
    state.requestedPosition = "home"
    
    log.debug "Camera moving to ${state.requestedPosition}."
    
    homeCmd()
}

def presetOne() {
    state.requestedPosition = "presetOne"
    
    log.debug "Camera moving to ${state.requestedPosition}."
    
    presetCmd(CameraPresetOne)
}

def presetTwo() {
    state.requestedPosition = "presetTwo"
    
    log.debug "Camera moving to ${state.requestedPosition}."
    
    presetCmd(CameraPresetTwo)
}

def presetThree() {
    state.requestedPosition = "presetThree"
    
    log.debug "Camera moving to ${state.requestedPosition}."
    
    presetCmd(CameraPresetThree)
}
